...
pub fn filter_mutations(
    mut bcf_positions: Vec<(String, u32)>,
    mutations: PathBuf,
) -> Result<Vec<(String, u32)>> {
    let mutations_file = File::open(mutations)?;
    let bcf_reader = BufReader::new(mutations_file);
    let mut to_remove = Vec::new();

    for line in bcf_reader.lines() {
        let line = line?;
        if !line.starts_with('#') {
            // Skip comment lines
            ...
            if fields.len() >= 2 {
                if let [chrom, pos, ..] = &fields[..] {
                    ...
                    if let Ok(pos) = pos.parse::<u32>() {
                        if bcf_positions.contains(&(chrom.to_string(), pos)) {
                            to_remove.push((chrom.to_string(), pos));
                            println!("bcf filtering: Chrom {:?} Pos {:?}", chrom, pos);
                        }
                    }
                }
            }
    ...
    bcf_positions.retain(|(chrom, pos)| !to_remove.contains(&(chrom.to_owned(), *pos)));

    Ok(bcf_positions)
}

pub fn filter_inconsistent(
    bcf_positions: Vec<(String, u32)>,
    consistent: PathBuf,
) -> Result<Vec<(String, u32)>> {
    ...
    let mut bcf_positions_filtered = Vec::new();
    ...
    for bed_line in bedgraph_reader.lines() {
        ...
        if let [chrom_orig, start, end, ..] = &bed_fields[..] {
            ...
            for pos in start..end {
                if bcf_positions.contains(&(chrom.to_string(), pos)) {
                    bcf_positions_filtered.push((chrom.to_string(), pos));
    ...
    Ok(bcf_positions_filtered)
}

pub fn write_filtered_candidates(
    output: Option<PathBuf>,
    filtered_bcf_positions: Vec<(String, u32)>,
) -> Result<()> {
    let mut bcf_header = Header::new();
    for contig_id in filtered_bcf_positions
        .clone()
        .into_iter()
        .map(|(contig, _)| contig)
        .collect::<Vec<String>>()
    ...
    for (contig, pos) in filtered_bcf_positions {
        let rid = bcf_writer
            .header()
